...
let
  ...
  cfg = config.xsession.windowManager.sway;
    ...
    options = {
      ...
      notification = mkOption {
        ...
        description = ''
          ...
          See <option>--no-startup-id</option> option description in the sway user guide.
        '';
      };
      ...
      workspace = mkOption {
        ...
        description = ''
          ...
          Use <varname><link linkend="opt-xsession.windowManager.sway.config.assigns">xsession.windowManager.sway.config.assigns</link></varname>
  ...
  barModule = types.submodule {
    options = {
      ...
      position = mkOption {
        ...
        default = "bottom";
        description = "The edge of the screen swaybar should show up.";
      };
      ...
      command = mkOption {
        type = types.str;
        default = "${cfg.package}/bin/swaybar";
        defaultText = "swaybar";
        description = "Command that will be used to start a bar.";
        example = "\${pkgs.sway-gaps}/bin/swaybar -t";
      };

      statusCommand = mkOption {
        type = types.str;
        default = "${pkgs.swaystatus}/bin/swaystatus";
      ...
      colors = mkOption {
        ...
        description = ''
          ...
          See <link xlink:href="https://swaywm.org/docs/userguide.html#_colors"/>.
        '';
      };
    ...
    options = {
      ...
      window = mkOption {
        type = types.submodule {
          options = {
            titlebar = mkOption {
              type = types.bool;
              default = cfg.package != pkgs.sway-gaps;
              defaultText = "xsession.windowManager.sway.package != nixpkgs.sway-gaps (titlebar should be disabled for sway-gaps)";
            ...
            commands = mkOption {
              ...
              description = ''
                ...
                See <option>for_window</option> swaywm option documentation.
              '';
        ...
        type = types.submodule {
          options = {
            ...
            modifier = mkOption {
              ...
              defaultText = "sway.config.modifier";
          ...
          options = {
            newWindow = mkOption {
              ...
              description = ''
                ...
                See <link xlink:href="https://swaywm.org/docs/userguide.html#focus_on_window_activation"/>
              '';
              example = "none";
            };
            ...
            forceWrapping = mkOption {
              ...
              description = ''
                ...
                See <link xlink:href="https://swaywm.org/docs/userguide.html#_focus_wrapping"/>
              '';
            };
      ...
      keybindings = mkOption {
        ...
        default = mapAttrs (n: mkOptionDefault) {
          "${cfg.config.modifier}+Return" = "exec sway-sensible-terminal";
          ...
          "${cfg.config.modifier}+Shift+e" = "exec sway-nagbar -t warning -m 'Do you want to exit sway?' -b 'Yes' 'sway-msg exit'";
        ...
        description = ''
          ...
          See <link xlink:href="https://swaywm.org/docs/userguide.html#keybindings"/>.
          </para><para>
        ...
        example = literalExample ''
          let
            modifier = xsession.windowManager.sway.config.modifier;
          in

          lib.mkOptionDefault {
            "''${modifier}+Return" = "exec sway-sensible-terminal";
      ...
      colors = mkOption {
        ...
        description = ''
          ...
          See <link xlink:href="https://swaywm.org/docs/userguide.html#_changing_colors"/>.
        '';
      };
      ...
      startup = mkOption {
        ...
        description = ''
          ...
          See <link xlink:href="https://swaywm.org/docs/userguide.html#_automatically_starting_applications_on_sway_startup"/>.
        '';
        example = literalExample ''
          [
      ...
      gaps = mkOption {
        ...
        default = null;
        description = ''
          swaygaps related settings.
          Note that sway-gaps package should be set for this options to take effect.
        '';
      };
    };
  };
  ...
  startupEntryStr = { command, always, notification, workspace, ... }: ''
    ${if always then "exec_always" else "exec"} ${
      ...
      if (workspace == null) then
        ...
        "sway-msg 'workspace ${workspace}; exec ${command}'"
    }
  '';

  configFile = pkgs.writeText "sway.conf" ((if cfg.config != null then with cfg.config; ''
    font pango:${concatStringsSep ", " fonts}
  ...
  options = {
    xsession.windowManager.sway = {
      ...
      package = mkOption {
        ...
        example = "pkgs.sway-gaps";
        description = ''
          sway package to use.
          If 'sway.config.gaps' settings are specified, 'pkgs.sway-gaps' will be set as a default package.
        '';
      };
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      xsession.windowManager.command = "${cfg.package}/bin/sway";
      xdg.configFile."sway/config" = {
        source = configFile;
        onChange = ''
          swaySocket=''${XDG_RUNTIME_DIR:-/run/user/$UID}/sway/ipc-socket.*
          if [ -S $swaySocket ]; then
            echo "Reloading sway"
            $DRY_RUN_CMD ${cfg.package}/bin/sway-msg -s $swaySocket reload 1>/dev/null
          fi
        '';
      };
    }
    ...
    (mkIf (cfg.config != null) {
      xsession.windowManager.sway.package = mkDefault (
        if (cfg.config.gaps != null) then pkgs.sway-gaps else pkgs.sway
      );
    })

    (mkIf (cfg.config != null && (any (s: s.workspace != null) cfg.config.startup)) {
      warnings = [
        ("'xsession.windowManager.sway.config.startup.*.workspace' is deprecated, "
          + "use 'xsession.windowManager.sway.config.assigns' instead."
